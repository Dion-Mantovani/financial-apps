...
export function dashboardController() {
  return {
    ...
    async fetchData() {
      ...
      try {
        ...
        if (this.wallets.length > 0) {
          this.customQuickForm.wallet_id = this.wallets[0].id;
        }
        const defaultCats = this.filteredCustomCategories;
        if (defaultCats.length > 0) {
          this.customQuickForm.category_id = defaultCats[0].id;
        }
    ...
    get monthlyGrowth() {
      ...
      // 1. Cek apakah ada transaksi sebelum bulan ini
      ...
      // 2. Hitung total pemasukan & pengeluaran bulan ini saja
      ...
      this.transactions.forEach((tx) => {
        ...
        if (txDateStr.startsWith(currentMonthPrefix)) {
          if (tx.type === 'income') incomeThisMonth += Number(tx.amount);
          if (tx.type === 'expense') expenseThisMonth += Number(tx.amount);
        }
      });

      // Saldo Awal Bulan = Saldo Sekarang - (Income Bulan Ini - Expense Bulan Ini)
    ...
    // Ganti getter weeklyExpensesData di dashboard.controller.ts dengan kode ini:
    get weeklyExpensesData() {
      const daysName = ['Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab', 'Ming'];
      const now = new Date();

      // Ambil day of week lokal (0: Minggu, 1: Senin, ..., 6: Sabtu)
      const currentDayOfWeek = now.getDay();
      // Hitung jarak ke hari Senin terdekat di minggu ini
      const distanceToMonday =
        currentDayOfWeek === 0 ? 6 : currentDayOfWeek - 1;

      // Tanggal Senin minggu ini
      ...
      // Helper format YYYY-MM-DD menggunakan WAKTU LOKAL (bukan UTC/toISOString)
      ...
      if (this.transactions && this.transactions.length > 0) {
        this.transactions.forEach((tx) => {
          ...
          // Ambil YYYY-MM-DD murni dari data transaksi
          const txDateStr = tx.transaction_date
            ? tx.transaction_date.split('T')[0]
            : '';
          ...
          if (matched) {
            matched.amount += Number(tx.amount);
          }
        });
      }
    ...
    renderWeeklyChart() {
      ...
      // Kalkulasi Batas Maksimum Y-Axis (Minimal 30.000)
      const realMax = Math.max(...seriesData, 0);
      const yAxisMax = Math.max(realMax, 30000);

      // LOGIKA BARU: Tentukan index hari ini (0: Sen, 1: Sel, ..., 5: Sab, 6: Ming)
      const now = new Date();
      const currentDay = now.getDay();
      const todayIndex = currentDay === 0 ? 6 : currentDay - 1; // Konversi ke urutan index array grafik

      // Pewarnaan: Hanya pilar HARI INI yang berwarna Biru Utama (#2563eb), sisanya Biru Soft (#bfdbfe)
      ...
      const options: ApexCharts.ApexOptions = {
        ...
        plotOptions: {
          bar: {
            borderRadius: 6,
            columnWidth: '40%',
            distributed: true, // Wajib true agar array warna 'colors' bisa diterapkan per batang
          },
        },
        colors: colors,
